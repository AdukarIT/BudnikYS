...
let californiaGrowPopulation = data.filter(function(item, index) {

    return (item.state === "California") && (parseInt(item.growth_from_2000_to_2013) > 0)
});

console.log(californiaGrowPopulation);

/* 4 */

function peopleFrom25In30 () {
    let peopleArr = data.filter(function(item) {
        return (parseInt(item.latitude) >= 25) && (parseInt(item.latitude) <= 30)
    });
        let sum = 0;
        peopleArr.forEach(function(item) {
            sum += (parseInt(item.population));
        });
        return sum;
}

console.log(peopleFrom25In30());

/* 5 */ 

let citiesD = data.filter(function(item) {
    return item.city[0] === "D";
})

citiesD = citiesD.sort(function(item, item2) {
    let a = item.city;
    let b = item2.city;
